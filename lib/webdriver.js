...
var npmScript = require('./npm-script'),
...
Provider.prototype.startServer = function (done) {
   ...
   console.log('provider: installing selenium server');
   npmScript('install-selenium', function() {
      ...
      selenium.start({
         ...
         child.stdout.on('data', function (data) {
            console.log('provider stdio: ' + data.toString());
         });
...
Provider.prototype.buildDriver = function (done) {
   console.log('provider: installing webdriver');
   var self = this;
   npmScript('install-webdriver', function() {
      var webdriverio = require('webdriverio');

      console.log('provider: building webdriver');
      self._driver = webdriverio
...
var Checker = function (driver, config) {
   ...
   var defaults = {
         ...
         timeout: 180000,//Таймаут, по истечении которого не имеет смысла больше ждать
   ...
   this.start = function (callback) {
      ...
      driver.timeoutsImplicitWait(config.delay, function () {
         console.log('provider: starting timer for ' + config.interval + 'ms');
