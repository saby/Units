   ...
   getReport: function (done, fail) {
      ...
      checker.start(function (checksDone) {
         ...
         driver.isExisting('body.tests-finished', function (err, isExisting) {
            ...
            console.log('loader: retrieving report');
            driver.getValue('#report', false, function (err, text) {
               if (err) {
                  fail(err);
               }
               console.log('loader: report retrieved');
               done(text);
            });
         });
         
         //Проверяем исключения
         driver.isExisting('#exception', function (err, isExisting) {
            console.log('loader: check exception - ' + isExisting);
            if (!isExisting) {
               return;
            }
            
            console.log('loader: web page throw an exception, getting text');
            checksDone();
            var text = driver.getText('#exception').then(function(text) {
