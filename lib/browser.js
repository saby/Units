...
class Loader {
    ...
    * @param {String} pathToReport Path to xml report
    * @return {Promise}
    */
   getScreenshot(pathToReport) {
      return new Promise((resolve, reject) => {
         let driver = this.provider.driver;
         const pathToDirReport = pathToReport.slice(0, pathToReport.lastIndexOf('/'));

         if (!fs.existsSync(pathToDirReport)) {
            fs.mkdirSync(pathToDirReport);
         }

         const fileName = `${pathToDirReport}/${Date.now()}.png`;

         logger.log(LOG_TAG, `Taking screenshot ${fileName}`);
         driver.saveScreenshot(fileName).then((result) => {
         ...
         driver.$('body.tests-finished').then((selector) => {
            selector.isExisting().then(isExisting => {
               ...
               driver.$('#report').then((report) => {
                  report.getValue(false).then(text => {
                     logger.log(LOG_TAG, `Report of ${text.length} symbol(s) retrieved`);
                     this.text = text;
                     resolve(true);
                  }).catch(reject);
               }).catch(reject);
...
exports.run = function(config) {
   ...
   loader.start(config.url).then(() => {
      ...
      loader.getReport().then(reportText => {
         ...
         //Logging report
         logger.log(LOG_TAG, 'Here are report contents below');
         ...
         loader.getScreenshot(config.reportFile).then(() => {
