...
const Provider = require('./webdriver');
 ...
 */
class Loader {
   constructor(provider, cfgProvider) {
      const Provider = require(provider === 'Selenium' ? './webdriver' : './Chrome');

      this._provider = new Provider(cfgProvider);
   }
   ...
   start(url) {
      return new Promise((resolve, reject) => {
         this.provider.startUp().then(() => {
            loadUrl(this.provider, url).then(resolve).catch(reject);
         }).catch(reject);
      });
   }
