...
function getConfig() {
   ...
   if (process.argv.length > 1) {
      let configArgv = process.argv.filter((item) => (item.includes('--config') || item.includes('--configUnits')));
      console.log('--------------' + JSON.stringify(configArgv));
      if (configArgv.length > 0) {
         let configPath = configArgv[0].split('=')[1];
         console.log('--------------configPath' + JSON.stringify(configPath));
         if (!configPath) {
            console.log('--------------configPath+1' + JSON.stringify(process.argv[process.argv.indexOf(configArgv[0]) + 1]));
            configPath = process.argv[process.argv.indexOf(configArgv[0]) + 1];
         }
         current = require(path.join(process.cwd(), configPath));
      }
   }
   let config = Object.assign({}, defaults, current);
