...
exports.node.pathTo = function(module) {
   var paths = [
         path.join(
            path.resolve(path.join(__dirname, '..')), 'node_modules', module, 'package.json'
         ),
         path.join(
            process.cwd(), 'node_modules', module, 'package.json'
         )
      ],
      item,
      i;
   ...
   for (i = 0; i < paths.length; i++) {
      item = paths[i];
      try {
         sysfs.accessSync(item, sysfs.constants.F_OK);
      } catch (e) {
         console.log(e);
         continue;
      }
