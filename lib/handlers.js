...
const {getWsConfig, isWsExists, isWsAppExists} = require('./ws/wsConfig');
...
function setup(config) {
   return (req, res) => {
      try {
         ...
         let wsExists = isWsExists(config.root);
         if (config.moduleType == 'amd') {
            startupScripts.push(wsExists ? `${wsConfig.appRoot}/RequireJsLoader/require.js` : 'node_modules/requirejs/require.js');
         }
         if (config.moduleType == 'amd' && wsExists) {
            startupScripts.push(`${wsConfig.appRoot}/RequireJsLoader/config.js`);
         }
