...
const {getRequireJsLoaderPath, getRequireJsConfigPath, getWsConfig} = require('./ws/wsConfig');
...
function setup(config) {
   return (req, res) => {
      try {
         ...
         let requireJsLoaderPath = getRequireJsLoaderPath(config.root);
         if (config.moduleType == 'amd') {
            startupScripts.push(requireJsLoaderPath ? `${requireJsLoaderPath}/require.js` : 'node_modules/requirejs/require.js');
         }
         if (config.moduleType == 'amd' && requireJsLoaderPath) {
            startupScripts.push(getRequireJsConfigPath(config.root));
         }
