...
function setup(config) {
   return (req, res) => {
      try {
         ...
         let wsLoad = fs.existsSync(path.join(config.root, 'WS.Core/WS.Core.s3mod'));
         ...
         if (config.moduleType == 'amd') {
            startupScripts.push(wsLoad ? `${config.ws}/ext/requirejs/require.js` : 'node_modules/requirejs/require.js');
         }
         if (config.moduleType == 'amd' && wsLoad) {
            startupScripts.push(`${config.ws}/ext/requirejs/config.js`);
         }
