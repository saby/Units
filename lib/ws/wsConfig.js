...
const REQUIREJS_CONFIG = 'ext/requirejs/config.js';
...
function getRequireJsLoaderPath(appRoot) {
  ...
  return moduleExists ? path.join(appRoot, REQUIREJS_LOADER_MODULE) : '';
}

function getRequireJsConfigPath(appRoot) {
  const loaderPath = getRequireJsLoaderPath(appRoot);
  return loaderPath ? path.join(loaderPath, REQUIREJS_CONFIG) : '';
}
