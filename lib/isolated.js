...
function setupRequireJs(requirejs, configPath, projectRootPath, wsRootPath, resourcesPath, contents) {
   let requirejsCfg = require(configPath)(
      projectRootPath,
      wsRootPath,
      resourcesPath,
      contents
...
exports.run = function(config) {
   ...
   try {
      ...
      if (e.originalError) {
         logger.error('Core initialization failed: ' + e);
      }
   ...
   unit.test.getList(
      ...
      try {
         ...
         logger.error('Module "' + test + '" failed with error: ' + e);
         logger.error(e.originalError || e);
         hasErrors = true;
      }
   });
