...
const {getWsConfig} = require('./ws/wsConfig');

const WS_CORE_CONFIG = 'ext/requirejs/config.js';
...
function prepareTestEnvironment(projectRootPath, dependencies, patchedRequire, wsRootPath, emulateBrowser) {
   ...
   const cfg = getWsConfig(projectRootPath, {
      resourcePath: '',
      wsPath: wsRootPath,
   ...
   try {
      //Setup RequireJS
      let requirejsConfigPath = path.resolve(path.join(projectRootPath, cfg.wsRoot, WS_CORE_CONFIG));
