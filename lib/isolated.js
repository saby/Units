...
function prepareTestEnvironment(projectRootPath, dependencies, patchedRequire, wsRootPath, emulateBrowser) {
   ...
   // Check if there is SbisEnv in the project and use it
   if (contents && contents.modules && contents.modules.SbisEnv) {
      requirejs('SbisEnv/StartApplication').default(filterMethods(cfg));
   }
...
/**
 * Фильтрует значени-функции в объекте
 * @param {Object} obj Объект, полный методов
 * @returns {Object} Объект без методов
 */
function filterMethods(obj) {
   var o = {};
