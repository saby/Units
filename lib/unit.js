...
/**
 * Return stat and path for file and if it symlink returns stats for the file on which it was created
 * @param itemPath
 * @returns {{stat: *, realPath: String}}
 */
function getStat(itemPath) {
   let stat = fs.lstatSync(itemPath);
   if (stat.isSymbolicLink()) {
      return getStat(fs.readlinkSync(itemPath));
   }
   return {
      realPath: itemPath,
      stat: stat
...
function lookingForFiles(parentDir, dirCallback, fileCallback, items, level) {
   ...
   fs.readdirSync(parentDir).forEach(itemName => {
      ...
      let {stat, realPath} = getStat(itemPath);
      if (stat.isDirectory()) {
         if (dirCallback(itemName, itemPath, level)) {
            lookingForFiles(realPath, dirCallback, fileCallback, items, 1 + level);
         }
      } else if (stat.isFile()) {
         let saveName = fileCallback(itemName, realPath, level);
         if (saveName) {
            items.push(saveName);
         }
      }
