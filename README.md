...
1. При подключении модулей (`*.module.js`) через плагин `js` для `requirejs`, в корне пакета должны находиться файлы `contents.js` и `contents.json` (в противном случае модули будут искаться в каталоге ресурсов WS).
